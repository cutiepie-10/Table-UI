...
export default function DataTable<TData=Record<string,number>>({columns, data}:DataTableProps<TData>){
    ...
    return (
            <div className='pr-6 !pb-12 !overflow-y-auto overflow-x-auto flex flex-col gap-6'>
                <Table.ScrollContainer maxHeight={900}>
                <Table withColumnBorders withTableBorder highlightOnHover stickyHeader stickyHeaderOffset={5} horizontalSpacing={'lg'} verticalSpacing={'md'} >
                    <Table.Thead>
                            {
                                table.getHeaderGroups().map((headerGroup)=>(
                                    <Table.Tr  key={headerGroup.id}>
                                        {headerGroup.headers.map((header)=>(
                                            <Table.Th key = {header.id} c={'white'} bg={'emerald.7'} colSpan={header.colSpan} >
                                                {
                                                    header.isPlaceholder ? null: (

                                                        <div
                                                            className={`flex flex-col !items-center !h-full p-1 ${header.column.getCanSort()
                                                        ...
                                                        >
                                                            ...
                                                            </div>
                                                            {
                                                                header.column.getCanFilter() ?(
                                                                    <div className='w-full flex-1 text-gray-600'>
                            ...
                            <Table.Tr key={row.id} >
                                {
                                    row.getVisibleCells().map((cell)=>(
                                            <Table.Td key={cell.id} style={{verticalAlign:'top'}}>
                                                ...
                                                hideLabel="less"
                                                styles={{
                                                    control:{fontSize:'12px',color:'whitesmoke', marginTop:'4px'}
                                                }}>
                                                    {
                                                    flexRender(
...
function Filter({
    ...
    (
        <TextInput
            ...
            radius={'md'}
            className='w-full'
            leftSection={<MagnifyingGlassIcon size={12} color='gray'/>}
